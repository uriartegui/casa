...
export async function registerPushToken(): Promise<PushTokenResult> {
  ...
  if (!PROJECT_ID) {
    devError('[Push] Project ID do EAS não encontrado.');
...
export async function unregisterPushToken(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      devWarn('[Push] Não foi possível remover token do backend:', fallbackErr?.message ?? err?.message ?? err);
    }
  }
}
